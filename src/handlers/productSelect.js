...
module.exports = (bot) => {
    ...
    bot.action(/^product_(\d+)$/, (ctx) => {
        ...
        // Clear the product-list message so the chat stays tidy.
        ...
        if (availableStock === 0) {
            return ctx.reply(messages.noStock, Markup.inlineKeyboard([
                [Markup.button.callback('↩️ Quay lại danh sách', 'refresh_products')],
            ]));
        }
