...
rsync -a \
  ...
  --exclude='data/db.sqlite' --exclude='data/db.sqlite-shm' --exclude='data/db.sqlite-wal' \
  --exclude='data/db.sqlite.fresh' --exclude='data/shop.db' \
...
The WordPress dump and uploads tarball are intentionally excluded — they will be referenced from the parent repo by the migration sub-project (#2). The mbbank-api pickled session state is excluded too: stale sessions tied to the parent's bank login would silently authenticate as that account.

**Critical: `.env` and other secrets must be excluded.** The parent's `.env` contains live `BOT_TOKEN`, `JWT_SECRET`, and `BANK_ACCOUNT`. Carrying that into the fork would mean Task 12's smoke test (`node src/index.js`) launches the parent's production bot under fork code. The fork must start with `.env` regenerated from `.env.example` using a brand-new bot token registered specifically for Taikhoantenhat.

The `**/__pycache__` and `**/*.pyc` patterns use rsync's recursive glob to catch nested `__pycache__` directories (e.g., `mbbank-api/mbbank-mh/mbbank-api/app/__pycache__/`), which the previous non-anchored exclude missed.

- [ ] **Step 3: Strengthen .gitignore for the fork**

The parent's `.gitignore` ignores `data/*.db` but the project actually writes `data/db.sqlite*`. It also doesn't ignore Python bytecode, TypeScript build info, or stray local secrets. Append the missing patterns before the initial commit:
...
(`data/*.db` matches `shop.db`-style names from the parent; the new lines cover the project's actual `db.sqlite` filenames and Python/TS build cruft. Duplicates are harmless.)
...
Expected: a single commit on `main` with all parent files (minus excludes). Verify `git ls-files | grep -E "(\\.env$|\\.pyc$|tsbuildinfo|__pycache__)"` returns nothing.

- [ ] **Step 5: Verify the fork boots dependencies**
...
- [ ] **Step 6: Commit the lockfiles if they changed**
