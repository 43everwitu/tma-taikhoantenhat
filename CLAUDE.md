...
- `src/services/discountService.js` — discount validation, global discount selection, best-code resolution
...
- `web/src/app/(miniapp)/components/AuthBoundary.tsx` — renders public catalog immediately; Mini App auth resolves in background and falls to `guest` outside Telegram
- `web/src/app/(miniapp)/components/AnnouncementCarousel.tsx` — compact home announcement rail; backend strips URLs/emojis for display
- `web/src/app/(miniapp)/components/DiscountCodeMeta.tsx` — inline discount code copy chip + auto/custom/hidden subtitle rendering
...
- **Noindex**: This app is not meant for public search indexing. Keep `web/src/app/robots.ts`, `metadata.robots`, Next `X-Robots-Tag`, and Express `X-Robots-Tag` in place for all HTML/API/uploads.
...
- **Discounts**: `/admin/discounts` manages manual/global discount codes (sidebar currently reuses `products.read`). `POST /admin/discounts/:id/notify` broadcasts a global code via bot/app messaging.

## Discounts

- Base table: `discount_codes` (migration 023) with global extensions from migrations 032/035/036: `is_global`, `app_message`, `bot_message`, `notify_title`, `app_meta_mode`, `app_meta_text`.
- `discountService.findActiveGlobal()` returns the active usable global code; creating/updating a code with `isGlobal: true` clears other global flags so there is only one active store-wide candidate.
- `discountService.resolveBestForOrder(rawCode, subtotal, userId)` compares active global vs manual code and chooses the larger discount. Checkout applies a manual discount only to the first order in a cart batch.
- Public API: `GET /api/v1/discounts/global` for banners; product list/detail pricing applies global sale fields; `GET /api/v1/home` includes `globalDiscount`.
- Mini App rendering: `DiscountCodeMeta` shows the copy chip and subtitle. `app_meta_mode='auto'` shows the computed label, `custom` shows `app_meta_text`, and `hidden` suppresses the subtitle.

## Message templates

- Table `message_templates(key, channel, label, variables, body, default_body, is_enabled, updated_at)`. 20 rows (3 `web.*` dual-channel mirrors added in migration 038).
...
- Optional list (11 keys): `welcome`, `payment_expired`, `bot.expiry_reminder`, `bot.stock_replenished`, `bot.product_new`, `bot.product_updated`, `web.stock_replenished`, `web.product_new`, `web.product_updated`, `admin.delivered`, `admin.low_stock`. Admin toggles via UI ON/OFF pill (`PUT /admin/messages/:key/toggle`).
- **Dual-channel (bot + Mini App)**: notifications sent to both Telegram chat and the in-app `notifications` table render from two separate templates so admins can word them differently — `bot.*` for the chat, `web.*` for the in-app body. `notify()`/`broadcast()` take an optional `webBody` (`undefined` ⇒ reuse the bot body). Applies to stock-replenished + product new/updated. `notifyStockReplenished` targets **all** `users` (everyone who started the bot), not per-product `product_follows`.
...
- Home data: `GET /api/v1/home` returns categories, featured, newest, announcements, and global discount in one request. Product list DTOs intentionally omit `longDescription`/`usageInstructions`; detail route owns rich content.
- Featured products: `products.is_featured`, public `GET /products/featured`, admin `PATCH /admin/products/:id/featured`, and the Mini App home "Sản phẩm nổi bật" rail.
...
| `v0.29-templates-prune` | templates — drop 14 unused seed rows (TMA covers payment/order flows), neutral pro tone (no Onii-chan / nhé / ~), per-template ON/OFF via `is_enabled` + `CORE_TEMPLATE_KEYS`, `renderIfEnabled` skips disabled sends, `/admin/messages` shows ON/OFF + "Bắt buộc" badge. 16 templates total at prune time. |
...
| `v0.33-admin-stock-poller` | admin edit modal (`page.tsx` + `EditModal.tsx`) + global `require_2fa_all` settings toggle (`twofaPolicy.js`); low-stock alerts route via `settings.low_stock_chat_id`/`low_stock_thread_id` with per-product threshold falling back to `settings.low_stock_alert_threshold` (`lowStockQuery.js`); live-read `payment_poll_interval_seconds` + `auto_payment_enabled` (`pollerConfig.js`); poller rescans recently-expired orders for late bank transfers (`orderService.getRecentlyExpired/markRecoveredPaid`) + hard-deletes expired >24h (`cleanupExpiredOrders`, indexed by migration 030). |
| `v0.34-global-discounts` | migrations 032/035/036: global discounts with auto-apply best-code checkout, admin `/admin/discounts`, app/bot notification copy, `notify_title`, Mini App `DiscountCodeMeta` copy chip, and `app_meta_mode` auto/custom/hidden subtitle. Product notify templates `bot.product_new`/`bot.product_updated`; featured products and home `AnnouncementCarousel`. |
| `v0.35-noindex-perf` | full noindex stack (`robots.ts`, metadata robots, Next + Express `X-Robots-Tag`), lean catalog DTOs, request-scoped global discount, aggregated stock SQL + indexes, `GET /home`, detail related/recent payloads, `/orders/batch`, QuickBuy cache reuse, fewer priority images, and faster guest rendering. |

## Pending follow-ups

- Refund route `POST /api/v1/admin/orders/:id/refund`.
